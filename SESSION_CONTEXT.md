...
- ユーザー要望「運用が複雑になってきたので、ダッシュボードに毎月の運用手順のサマリも記載して欲しい」に対応。[generate_dashboard.py](scripts/generate_dashboard.py)に`_build_monthly_ops_section()`を追加し、ページ上部（投資シグナルのガイドの下）に開閉式の「毎月の運用手順（サマリ）」を表示（F-25）。自動で行われること／毎月の作業3ステップ／Tier到達時の対応／毎月見る数字（サテライト目標・S&P500基準は`settings.json`から表示）／注意。別枠積立の積立日は確定情報がないため「SBI証券の積立設定日（実績では毎月10日前後）」と記載。幅375pxで開閉動作・横はみ出しなしを確認。直前にコミット・プッシュした1コマンド化（`08c33bb`）に続く変更
- ユーザーがSBI証券の積立設定のスクリーンショットを提供：**別枠積立＝クレジットカード払い**（オルカン30,000円＝NISAつみたて、Tracers20,000円＝NISA成長、いずれも毎月9日発注）、**攻撃フェーズの毎月積立＝現金・NISA成長**（Tracers21,765円・SOX14,706円・eMAXIS S&P500 11,765円・オルカン10,588円、毎月23日発注、約定は25日前後）。これまで私が「つみたて投資枠＝別枠積立」と説明・実装していたのは不完全で、**Tracersの別枠積立は預り区分が成長投資枠のため、預り区分による判定だと攻撃フェーズと誤判定される**ことが判明（CSVに決済方法の列がない）。対応：[import_sbi_history.py](scripts/import_sbi_history.py)で銘柄ごとの別枠積立の固定金額（`settings.json`の`sbi_import.side_fixed_amounts`＝オルカン30,000円・Tracers20,000円）に一致する約定を別枠積立と判定、区分の推定を約定日23〜27日（`recurring_day_range`）に拡張。毎月の積立設定を`monthly_schedule`（表示専用）に登録し、ダッシュボードの「毎月の運用手順」に発注日・約定日・金額を表示（前回の「別枠積立は毎月10日前後」の推測記載を確定情報に置き換え）。検証：合成行でTracers成長20,000円→side／21,765円→attack、オルカンつみたて30,000円→side、23〜27日→定期積立、範囲外→未分類、実CSVは全19件が重複として不変。**運用上の注意**: 積立金額を変えたら`side_fixed_amounts`と`monthly_schedule`も更新する
